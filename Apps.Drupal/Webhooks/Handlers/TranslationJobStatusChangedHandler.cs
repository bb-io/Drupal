...
public class TranslationJobStatusChangedHandler(InvocationContext invocationContext)
    : AppInvocable(invocationContext), IWebhookEventHandler
{
        ...
        Dictionary<string, string> values)
    {
        var request = new ApiRequest("/api/tmgmt/blackbird/webhooks", Method.Post, creds)
            .AddJsonBody(new
            {
                ...
                events = new[] { WebhookTopics.TranslationJobStatusChanged }
            });
        ...
        Dictionary<string, string> values)
    {
        ...
        var subscription = subscriptions.FirstOrDefault(item =>
            ...
            item.Events.SequenceEqual([WebhookTopics.TranslationJobStatusChanged]));
